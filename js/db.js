...
function _init() {
  ...
  if (!_isConfigured()) {
    console.warn('[DB] Firebase not configured — using localStorage. Fill in js/firebase-config.js.');
    return;
  }
  ...
  try {
    ...
    console.log('%c[DB] Firebase Firestore connected ✅', 'color:#4caf82;font-weight:bold');
  } catch (e) {
    console.error('[DB] Init failed, falling back to localStorage:', e.message);
    _db = null; _auth = null; _ready = false;
  }
}
...
const DB = {
  ...
  async getRegs() {
    ...
    if (_ready) {
      ...
      } catch (e) { console.error('[DB] getRegs failed:', e); }
    }
  ...
  async saveReg(reg) {
    ...
    const data = { ...reg, timestamp: reg.timestamp || new Date().toISOString() };
    if (_ready) {
      try {
        await _db.collection('registrations').doc(data.id).set(data);
        return data.id;
      } catch (e) { console.error('[DB] saveReg Firebase failed:', e); }
    }
  ...
  async updateReg(id, patch) {
    _init();
    if (_ready) {
      try { await _db.collection('registrations').doc(id).update(patch); return; } catch (e) { console.error('[DB] updateReg failed:', e); }
    }
  ...
  async deleteReg(id) {
    _init();
    if (_ready) {
      try { await _db.collection('registrations').doc(id).delete(); return; } catch (e) { console.error('[DB] deleteReg failed:', e); }
    }
  ...
  async getRegById(id) {
    ...
    if (_ready) {
      ...
      } catch (e) { console.error('[DB] getRegById failed:', e); }
    }
  ...
  async getRegsByEmail(email) {
    ...
    if (_ready) {
      try {
        const snap = await _db.collection('registrations').where('email', '==', lc).get();
        return snap.docs.map(d => ({ ...d.data(), id: d.id }));
      } catch (e) { console.error('[DB] getRegsByEmail failed:', e); }
    }
  ...
  async addLog(entry) {
    _init();
    /* FIX-3: ISO timestamp */
    const doc = { ...entry, timestamp: new Date().toISOString(), userAgent: (navigator.userAgent || '').slice(0, 120) };
    if (_ready) {
      try { await _db.collection('loginLogs').add(doc); return; } catch (e) { console.error('[DB] addLog failed:', e); }
    }
  ...
  async getLogs() {
    ...
    if (_ready) {
      try {
        ...
        return snap.docs.map(d => ({ ...d.data(), id: d.id }));
      } catch (e) { console.error('[DB] getLogs failed:', e); }
    }
  ...
  async clearLogs() {
    ...
    if (_ready) {
      try {
        ...
        await batch.commit();
        return;
      } catch (e) { console.error('[DB] clearLogs failed:', e); }
    }
  ...
  async getUsers() {
    ...
    if (_ready) {
      ...
      } catch (e) { console.error('[DB] getUsers failed:', e); }
    }
  ...
  async saveUser(user) {
    ...
    if (_ready) {
      try { await _db.collection('users').doc(uid).set({ ...user, uid }, { merge: true }); return; } catch (e) { console.error('[DB] saveUser failed:', e); }
    }
    const arr = LS.get('wpsa_users') || [];
    const i   = arr.findIndex(u => u.uid === uid || u.email === user.email);
    if (i >= 0) arr[i] = { ...arr[i], ...user, uid }; else arr.push({ ...user, uid });
  ...
  async deleteUser(uid) {
    _init();
    if (_ready) {
      try { await _db.collection('users').doc(uid).delete(); return; } catch (e) { console.error('[DB] deleteUser failed:', e); }
    }
    LS.set('wpsa_users', (LS.get('wpsa_users') || []).filter(u => u.uid !== uid));
  },
  ...
  async getSettings() {
    ...
    if (_ready) {
      ...
      } catch (e) { console.error('[DB] getSettings failed:', e); }
    }
  ...
  async saveSettings(s) {
    _init();
    if (_ready) {
      try { await _db.collection('settings').doc('main').set(s); return; } catch (e) { console.error('[DB] saveSettings failed:', e); }
    }
